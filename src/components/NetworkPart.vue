<template>
    <div class="grid grid-cols-12 gap-2.5 pt-20">
        <h1 class="col-start-1 col-span-9 text-6xl text-blue font-semibold font-[raleway]"><span class="text-orange">{{ t.networkTitle.charAt(0) }}</span>{{ t.networkTitle.slice(1) }}</h1>
        <p class="col-start-1 col-span-8 mt-6 text-lg text-blue font-[raleway] font-semibold">{{ t.networkSubtitle }}</p>
    </div>

    <div class="grid grid-cols-12 gap-2.5 mt-20 items-center">
        <h2 class="col-start-1 col-span-3 text-4xl font-bold text-blue font-[raleway]">{{ t.networkHeading }}</h2>
    ...
    <div class="grid grid-cols-12 gap-2.5 mt-10 mb-20">
        <RouterLink v-for="member in filteredMembers" :key="member.id" :to="`/network/${member.slug}`" class="col-span-4 relative rounded-2xl overflow-hidden h-80 hover:scale-[1.02] transition-transform duration-300 cursor-pointer block">
    ...
    <svg v-else xmlns="http://www.w3.org/2000/svg" width="28" height="17" viewBox="0 0 5 3"><path fill="#000000" d="M0 0h5v1H0z"/><path fill="#dd0000" d="M0 1h5v1H0z"/><path fill="#ffce00" d="M0 2h5v1H0z"/></svg>
            ...
            <div class="absolute bottom-0 left-0 right-0 p-5 text-white font-[raleway]">
                ...
                <p class="text-sm font-light mb-4">{{ member[`description_${currentLanguage}`] || member.description }}</p>
    ...
    <span class="text-white text-xs font-[raleway] uppercase tracking-wider">{{ t.moreAbout }}</span>
</div>
            </div>
        </RouterLink>
    </div>
